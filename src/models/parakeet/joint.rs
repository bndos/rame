...
use super::config::ParakeetTdtConfig;
...
impl From<ParakeetTdtConfig> for TdtJointLayout {
    fn from(config: ParakeetTdtConfig) -> Self {
        Self {
            num_token_classes: config.num_token_classes,
            num_duration_classes: config.durations.len(),
        }
    }
}
    ...
    pub fn from_combined(logits: Tensor, layout: TdtJointLayout) -> RameResult<Self> {
        ...
        let Some((&width, leading_dimensions)) = dimensions.split_last() else {
            return Err(ModelError::InvalidTensorShape {
                name: "Parakeet joint logits".into(),
                expected: format!(
...
mod tests {
    use crate::RameError;
    use crate::models::ModelError;
    use crate::models::parakeet::ParakeetTdtConfig;
    use crate::tensor::Tensor;

    use super::{JointOutput, TdtJointLayout};

    #[test]
    fn splits_combined_joint_logits_without_changing_leading_dimensions() {
        let layout = TdtJointLayout::from(ParakeetTdtConfig::v0_6b_v3());
        let joint_logit_count = layout.joint_logit_count();
        let logits =
            Tensor::from_vec(vec![0.0f32; 2 * joint_logit_count], (2, joint_logit_count)).unwrap();

        let output = JointOutput::from_combined(logits, layout).unwrap();
    ...
    fn rejects_an_incompatible_joint_width() {
        ...
        let error = JointOutput::from_combined(logits, layout).unwrap_err();
