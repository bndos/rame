...
#[derive(Debug)]
#[doc(hidden)]
pub struct PpDocLayoutV3OnnxProcessor {
    inputs: Inputs,
...
impl Processor for PpDocLayoutV3OnnxProcessor {
...
mod tests {
    ...
    #[test]
    fn creates_pp_doclayout_v3_onnx_inputs() {
        let image = Image::from_rgb8(1, 1, vec![255, 127, 0]).unwrap();
        let processor = PpDocLayoutV3OnnxProcessor::new(
            Inputs::default(),
            Preprocess {
    ...
    #[test]
    fn creates_batched_pp_doclayout_v3_onnx_inputs() {
        let images = [
